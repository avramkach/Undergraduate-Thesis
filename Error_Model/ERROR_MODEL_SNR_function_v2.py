...
def ERROR_MODEL_SNR_v2(f, Zre, Zim, SNR):
    n = len(f)
    Z = Zre+Zim*1j
    ...
    for i in range(0, n):
        ...
        y = sin_wave(fi, t1, np.angle(Z[i]) + Ishift + Vshift, np.abs(Z[i]))
    ...
    #return cycles and SNR without noise think of the build
    if SNR == np.inf:
        return Zout
    else:
        return Zoutn
